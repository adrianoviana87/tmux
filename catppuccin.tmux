...
build_window_format() {
  ...
  if [ "$fill" = "none" ]
  then
    local show_left_separator="#[fg=$thm_window_left_separator_fg,bg=$thm_bg,nobold,nounderscore,noitalics]$window_left_separator"
    local show_number="#[fg=$thm_fg,bg=$window_number_bg]$number"
    local show_middle_separator="#[fg=$thm_fg,bg=$window_middle_separator_bg,nobold,nounderscore,noitalics]$window_middle_separator"
    local show_text="#[fg=$text_fg,bg=$text_bg]$text"
    local show_right_separator="#[fg=$window_right_separator_fg,bg=$thm_bg]$window_right_separator"
...
build_status_module() {
  ...
  if [ "$status_fill" = "icon" ]
  then
    local show_left_separator="#[fg=$color,bg=$thm_status_left_separator_bg,nobold,nounderscore,noitalics]$status_left_separator"

    local show_icon="#[fg=$color,bg=$thm_bg,nobold,nounderscore,noitalics]$icon "
    local show_text="#[fg=$thm_fg,bg=$thm_status_text_bg] $text"

    local show_right_separator="#[fg=$thm_status_right_separator_fg,bg=$thm_bg,nobold,nounderscore,noitalics]$status_right_separator"

    if [ "$status_connect_separator" = "yes" ]
    then
      local show_left_separator="#[fg=$color,bg=$thm_status_left_separator_bg,nobold,nounderscore,noitalics]$status_left_separator"
      local show_right_separator="#[fg=$thm_status_right_separator_fg,bg=$thm_gray,nobold,nounderscore,noitalics]$status_right_separator"
  ...
  then
    ...
    local show_right_separator="#[fg=$color,bg=$thm_status_right_separator_fg,nobold,nounderscore,noitalics]$status_right_separator"
  ...
  then
    if [ "$status_connect_separator" = "yes" ]
    then
      local show_right_separator="#[fg=$thm_status_right_separator_fg,bg=$color,nobold,nounderscore,noitalics]$status_right_separator"
    else
...
main() {
  ...
  # messages
  set message-style "fg=${thm_message_style_fg},bg=${thm_message_style_bg},align=centre"
  set message-command-style "fg=${thm_message_command_style_fg},bg=${thm_message_command_style_bg},align=centre"

  # panes
  set pane-border-style "fg=${thm_pane_border_style_fg}"
  set pane-active-border-style "fg=${thm_active_border_style_fg}"
  ...
  local status_modules=$(get_tmux_option "@catppuccin_status_modules" "git_info current_directory session")
  ...
  # --------=== Modes
  #
  setw clock-mode-colour "${thm_clock_mode_colour}"
  setw mode-style "fg=${thm_mode_style_fg} bg=${thm_mode_style_bg} bold"
